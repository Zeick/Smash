...
Escale = [1.96 20]; % Energy scale: 10^Escale GeV, log10(MZ/GeV) = 1.96
%opts = odeset('RelTol',1e-4,'AbsTol',1e-6);
[t, x] = ode45('rgeq',Escale,x0); % ODE-function, solution span, initial values, error tolerance
...
subplot(2,3,2);
plot(t,abs(yt)); hold on; plot(t, abs(yn)); xlabel('log_{10} \mu/GeV');
legend('{\fontsize{15}y_t}','{\fontsize{15}y_N}','Location','NorthEast');
subplot(2,3,3);
plot(t, abs(yb)); hold on; plot(t, abs(ytau)); plot(t, abs(yf)); plot(t, abs(yq)); xlabel('log_{10} \mu/GeV');
legend('{\fontsize{15}y_b}','{\fontsize{15}y_\tau}','{\fontsize{15}y_F}','{\fontsize{15}y_Q}','Location','NorthEast');
subplot(2,3,4);
plot(t, abs(lambdaH)); hold on; plot(t, 100*abs(lambdaS)); plot(t, 1000*abs(lambdaHS));
ylim([0 1.0]); xlabel('log_{10} \mu/GeV');
legend('{\fontsize{15}\lambda_H}','{\fontsize{15}100\lambda_S}','{\fontsize{15} 10^3\lambda_{HS}}','Location','NorthEast');
subplot(2,3,5);
plot(t, abs(muH)); hold on; plot(t, abs(muS)/10^2);
ylabel('GeV'); xlabel('log_{10} \mu/GeV');
legend('{\fontsize{15}m_H}','{\fontsize{15}0.01m_S}','Location','NorthEast');
h = suptitle('5m_N = v_\sigma = 10^{14} GeV, \lambda_S = 10^{-6}, \lambda_{HS} = 10^{-11}, Y_F = 0.05, Y_Q = 0.1');
