...
lambdaS0 = 3e-9;         % Scalar singlet self-coupling 5e-9
lambdaH0 = mh^2/(v^2) + lambdaHS0^2/lambdaS0;   % SM Higgs self-coupling at MZ + threshold correction
...
figure;
subplot(1,2,1);
plot(t, muH,'LineWidth',2); hold on; plot(t,muS/10^3,'LineWidth',2);
plot(SMt, SMmuH,'--b','LineWidth',2);
xlabel('log_{10} \mu/GeV'); ylabel('GeV');
legend('{\fontsize{15}m_H}','{\fontsize{15}m_\sigma/10^3}','{\fontsize{15}m_H (SM)}','Location','NorthWest');
set(gca,'XMinorTick','on','YMinorTick','on');
set(gca,'LineWidth',2,'TickLength',[0.025 0.025]);
set(gca,'FontSize',15);
ylim([0 200]);
...
%figure;
subplot(1,2,2);
plot(t, lambdaH,'LineWidth',2); hold on; plot(t, 10^3*lambdaHS,'LineWidth',2);
